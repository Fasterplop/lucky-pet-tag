  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (signInError) throw new Error(t('auth.invalidCredentials'));

        await redirectUser(authData.user?.email || email);
      } else {
        // Sign Up validation
        if (password !== confirmPassword) {
          throw new Error(t('auth.passMismatch'));
        }

        if (password.length < 6) {
          throw new Error(t('auth.passMinLength'));
        }
      ...
      <main className="flex-grow relative flex items-center justify-center py-16 px-6 z-10">
        <div className="w-full max-w-[520px]">
          <div className="bg-white rounded-3xl p-10 md:p-14 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.12)] border border-[#bec9c0]/10">
            <div className="flex flex-col items-center text-center mb-10">
              <div className="mb-6">
                <div className="flex flex-col items-center">
                  <div className="w-24 sm:w-28 h-auto mb-4 hover:opacity-80 transition-opacity">
                    <img
                      src="/logo.webp"
                      alt="Lucky Pet Tag"
                      ...
                      style={{
                        filter:
                          'brightness(0) saturate(100%) invert(48%) sepia(61%) saturate(417%) hue-rotate(104deg) brightness(89%) contrast(86%)',
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="mb-6">
              {error && (
                <div className="bg-[#ba1a1a]/10 border border-[#ba1a1a]/20 text-[#ba1a1a] p-4 rounded-2xl text-sm text-center font-medium">
                  {error}
                </div>
              )}
              {message && (
                <div className="bg-[#4D9E76]/10 border border-[#4D9E76]/20 text-[#4D9E76] p-4 rounded-2xl text-sm text-center font-medium">
                  {message}
                </div>
              )}
            </div>
            ...
            {isForgotPassword ? (
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-6">
                <div className="space-y-2 text-left">
                  <label className="text-[11px] font-bold text-[#3f4942] tracking-wider uppercase ml-1 block cursor-pointer">{t('auth.emailLabel')}</label>
                  <input
                    type="email"
                    required
                    value={email}
                ...
                <div className="space-y-2 text-left">
                  <div className="flex justify-between items-center ml-1">
                   <label className="text-[11px] font-bold text-[#3f4942] tracking-wider uppercase block cursor-pointer">{t('auth.passwordLabel')}</label>
                    {isLogin && (
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  <div className="space-y-2 text-left">
                    ...
                    {!isLogin &&
                      ...
                      password !== confirmPassword && (
                        <p className="text-[11px] text-[#ba1a1a] font-medium ml-2">
                          {t('auth.passMismatch')}
                        </p>
                      )}
                  </div>
                ...
                >
                  {loading ? (
                    <>
                      <span className="w-2.5 h-2.5 bg-[#4D9E76] rounded-full shadow-[0_0_8px_#4D9E76] animate-pulse"></span>
                      {t('auth.processing')}
                    </>
                ...
                <div className="relative my-10 flex items-center">
                  ...
                  <span className="px-4 text-[10px] font-bold text-[#bec9c0] uppercase tracking-[0.2em]">
                    {t('auth.or')}
                  </span>
                  <div className="flex-grow border-t border-[#bec9c0]/20"></div>
                </div>
                ...
                >
                  {isLogin ? t('auth.createNewBtn') : t('auth.signInExistingBtn')}
                </button>
              </form>
            )}
            ...
            <footer className="mt-12 text-center">
              <div className="flex items-center justify-center gap-6 text-[10px] font-bold text-[#bec9c0] uppercase tracking-widest">
                <a
                  href="https://luckypetag.com/privacy"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-[#4D9E76] transition-colors cursor-pointer"
                >
                  {t('auth.privacy')}
                </a>
                <span className="w-1 h-1 rounded-full bg-[#bec9c0]/30"></span>
                <a
                  href="https://luckypetag.com/terms"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-[#4D9E76] transition-colors cursor-pointer"
                >
                ...
                <span className="w-1 h-1 rounded-full bg-[#bec9c0]/30"></span>
                <a
                  href="https://luckypetag.com/help"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-[#4D9E76] transition-colors cursor-pointer"
                >
